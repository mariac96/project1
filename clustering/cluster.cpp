...
int main(int argc, char *argv[])
{
	...
	for(int t = 1; t <= (K+1); t++)
	{
		...
		// random uniformly distributed x [0, P(n−t)]
		// returns r ∈ [1, 2, ..., n−t]: P(r−1) < x <= P(r), P(0) = 0
	...
	if(method == "LSH")
		LSH(dataset.size(), number_of_vector_hash_tables,
		number_of_vector_hash_functions, dataset, outputFile, c, d, complete);
	else if( method == "Hypercube")
		hypercube(max_number_M_hypercube, number_of_hypercube_dimensions,
			number_of_probes, dataset, outputFile, c, d, complete);
	else if(method == "Classic")
		Lloyds(dataset, outputFile, c, complete);
	return 0;
}
