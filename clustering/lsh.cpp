...
void LSH(int vSize, int L, int k, std::vector<dVector>& dataset, 
	std::string& outputFile, std::vector<cluster> clust ,int d, bool complete)
{
	...
	while(flag == false)
	{
		...
		for(int i = 0 ; i < clust.size(); i++)
		{
			dVector c = clust.at(i).getCentre();
			vector<double> v1 = c.getVector();
			vector<int> buckets = clust.at(i).getBuckets();
	...
	for(int i = 0; i< dataset.size(); i++)
	{
		...
		dataset.at(i).mark();
		clust.at(minI).addToCluster(dataset.at(i));
	}
